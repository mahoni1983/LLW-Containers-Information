...
#Window icon    can not be icluded into one EXE file programm
window_icon = "barrel_icon.ico"
program_name = "LLW Containers Information v.1.0.1"
about_program_text = [program_name + "\nThe program is designed to work at Ignalina Nuclear Power Plant.\n"
                      "It shows information about sent for storage steel containers with long-live radioactive waste.\n"
                      "The program connects to Tracking database, collects and represents information for all sent for storage steel containers or for a single container (for entered barcode).\n\n"
...
query_curent =  "select StorageX_Cont as X, StorageY_Cont as Y, StorageZ_Cont as Z, ID_Cont as Container,  upDate_Cont " \
                "as Stored_date,  Waste_Cont as Waste, MassNetto_MST11CMS as Mass_netto, " \
                "DRSMean_MST11CMS	 as DR_mean, 	DRSMax_MST11CMS as DR_max, StartDate_MST11CMS as Measured, Remarks_Cont as Remark from " \
                "T_Cont, T_MST11CMS where Type_cont = 2 and  T_Cont.MeasID_MST110_Cont = T_MST11CMS.ID_MST11CMS order " \
                "by StorageX_Cont, StorageY_Cont, StorageZ_Cont"
#in (select ID_Cont from T_Cont where lastAction_Cont = 700700) "
# query to get info for btn_custom
query_custom = "select StorageX_Cont as X, StorageY_Cont as Y, StorageZ_Cont as Z, ID_Cont as Container,  upDate_Cont " \
                "as Stored_date,  Waste_Cont as Waste, MassNetto_MST11CMS as Mass_netto, " \
                "DRSMean_MST11CMS	 as DR_mean, 	DRSMax_MST11CMS as DR_max, StartDate_MST11CMS as Measured, Remarks_Cont as Remark from " \
                "T_Cont, T_MST11CMS where Type_cont = 2 and  T_Cont.MeasID_MST110_Cont = T_MST11CMS.ID_MST11CMS and ID_Cont = $to_replace$ " \
                "order " \
                "by StorageX_Cont, StorageY_Cont, StorageZ_Cont"

# to represent answer from SQL in appropriate form for two lang.
list_columns = [
    ["X", "Y", "Z", "Container", "Stored_date", "Waste", "Mass_netto", "DR_mean", "DR_max", "Measured", "Remark"],
    ["X", "Y", "Z", "Container", "Stored date", "Waste", "Mass netto,\n kg", "DR mean,\n mSv/h", "DR max,\n msV/h", "Measured date", "Remark"],
    ["X", "Y", "Z", "Контейнер", "Дата установки", "Отходы", "Масса нетто,\n кг", "Ср. МЭД,\n мЗв/ч", "Макс. МЭД,\n мЗв/ч", "Дата измерения", "Заметки"]
]

dict_columns_to_summ = {"Z": 'each count', "Container": 'count', "Stored_date": 'range date', "Waste": 'each count', "Mass_netto": 'sum', "DR_mean": 'range', "DR_max": 'range', "Measured": 'range date'}
#\
...
def show_info_about_custom_container(custom_container):
    ...
    if connector == None:  # failed to connect to DB
        ...
        cursor = connector.cursor()
        try:
            cursor.execute(query_custom.replace('$to_replace$', custom_container))
        except:
